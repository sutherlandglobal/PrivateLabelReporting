...
public final class Roster extends Report 
{
	...
	public void load()
	{
		...
		Vector<String[]> rawRoster = startReport();
	...
	{		
		...
		try 
		{
			...
			//AGENT_STACK_REPORT schedule hit the databases pretty hard. if we're only looking for one user
			if(getParameter(AGENT_NAME_PARAM) == null)
			{
			...
			{
				fullName = name.getValue().getAttrData(FULLNAME_ATTR).firstElement();
				empID = name.getValue().getAttrData(EMP_ID_ATTR).firstElement();
				userID = name.getValue().getAttrData(USER_ID_ATTR).firstElement();
	 ...
	 */
	private Vector<String[]> getActiveSupportAndSalesRoster()
	{
		Vector<String[]> retval = getActiveSupportRoster();
	 ...
	 * @return	The roster fields.
	 */
	private Vector<String[]> getSupportAndSalesRoster()
	{
		Vector<String[]> retval = getSupportRoster();
	...
	public String getFullName(String userParameter)
	{
		...
		try
		{
			fullName = user.getAttrData(LASTNAME_ATTR).firstElement() + ", " + user.getAttrData(FIRSTNAME_ATTR).firstElement();
		}
		catch(NullPointerException e)
		{
	 ...
	 * 
	 */
	private Vector<String[]> getActiveSupportRoster() 
	{
	 ...
	 * 
	 */
	private Vector<String[]> getSupportRoster()
	{
		return dbConnection.runQuery
				(
	 ...
	 * 
	 */
	private Vector<String[]> getSalesRoster() 
	{
		return dbConnection.runQuery
				(
						queryFields +
	 ...
	 * 
	 */
	private Vector<String[]> getActiveSalesRoster() 
	{
		return dbConnection.runQuery
				(
						queryFields +
	...
	{
		return ReportParameterValidator.validateRosterType(getParameter(ROSTER_TYPE_PARAM));
	}
