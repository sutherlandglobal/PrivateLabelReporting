...
class IVRCase extends CaseTicket
{
	...
	public function toHTML()
	{
		...
		foreach(parent::getFields() as $field)
		{
			...
			if(parent::getField($field))
			{
				...
				{
					$htmlString .= "<a href =\"\">" . parent::getField($field). "</a>";
				}
				else
				{
	...
	protected function calcSLADeadline()
	{
		...
		$dateFields = date_parse_from_format( "Y-m-d H:i:s",  parent::getField(IVRCase::DATE_FIELD));
		...
		echo "SLA Deadline for IVRCase ", parent::getField($this->idFieldName), " is ", $date->format("Y-m-d H:i:s"), PHP_EOL;
