 ...
 *
 */
public final class CallsPerCase extends Report  implements DataAttributes
{
	...
	protected ArrayList<String[]> runReport() throws ReportSetupException
	{
		...
		for(String grain : reportGrainData.getDatumIDList())
		{
			...
			if(!reportGrainData.getDatum(grain).addAttribute(UPDATED_CASES_ATTR) && !reportGrainData.getDatum(grain).addAttribute(CALL_VOL_ATTR) && !reportGrainData.getDatum(grain).addAttribute(CREATED_CASES_ATTR))
			{
				finalCallVolume = Statistics.getTotal(reportGrainData.getDatum(grain).getAttributeData(CALL_VOL_ATTR));
