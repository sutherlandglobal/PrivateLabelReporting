 ...
 *
 */
public final class UsedIssues extends Report implements DataAttributes
{
	private RemoteConnection dbConnection;
	private PrivateLabelRoster roster;
	private final String dbPropFile = DatabaseConfigs.PRIVATE_LABEL_PROD_DB;
	private final static Logger logger = Logger.getLogger(UsedIssues.class);
	...
	protected boolean setupReport() 
	{
		...
		try
		{
			reportName = UsedIssues.uiGetReportName();
			reportDesc = UsedIssues.uiGetReportDesc();
	...
	protected ArrayList<String[]> runReport() throws Exception
	{
		...
		String userID, reportGrain, soldIssues;
		...
		for(String[] row:  dbConnection.runQuery(ordersQuery))
		{
			...
			if(roster.hasUser(userID) )
			{
				...
				reportGrainData.getDatum(reportGrain).addData(USED_ISSUES_ATTR, soldIssues);
			}
		}
		...
		{
			numIssues = Statistics.getTotal(reportGrainData.getDatum(grain).getAttributeData(USED_ISSUES_ATTR));
