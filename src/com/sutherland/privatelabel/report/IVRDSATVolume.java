 ...
 *
 */
public final class IVRDSATVolume extends Report implements DataAttributes 
{
		...
		for(String[] row : dsatCaseReport.startReport())
		{
			...
			if(targetUserID != null && roster.hasUser(targetUserID) )
			{
				...
				if(isTimeTrendReport())
				{
					...
					reportGrain = DateFormatter.getFormattedDate(DateParser.convertSQLDateToGregorian(row[3]), timeGrain, dateFormat);
				}
				else //if(isStackReport())
				{
					//is stack report
