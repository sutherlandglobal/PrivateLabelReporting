	...
	protected ArrayList<String[]> runReport() throws ReportSetupException
	{
		...
		else
		{
			...
			for(String[] row : runner.getResults(SCH_ADH_ATTR))
			{
				...
				users.getDatum(row[0]).addAttribute(SCH_ADH_ATTR );
				users.getDatum(row[0]).addData(SCH_ADH_ATTR, row[1]);
			}
		...
		for(String userName : users.getDatumIDList())
		{
			...
			if(users.getDatum(userName).getAttributeData(SCH_ADH_ATTR) != null )
			{
				scheduleAdherence = Double.parseDouble(users.getDatum(userName).getAttributeData(SCH_ADH_ATTR).get(0));
			}
