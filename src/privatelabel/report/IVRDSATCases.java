	...
	protected ArrayList<String[]> runReport() throws Exception
	{
		...
		for(String[] surveyRow : dbConnection.runQuery(query))
		{
			...
			if(targetUserID != null && roster.hasUser(targetUserID) && surveyString.charAt(5) == '2')
			{
				...
				reportGrainData.getDatum(reportGrain).addAttribute(SURVEYS_ATTR);
				reportGrainData.getDatum(reportGrain).addObject(SURVEYS_ATTR, surveyData);
			}			
		}
		...
		{
			for(Object row : reportGrainData.getDatum(grain).getDatumObjects(SURVEYS_ATTR))
			{
				rowData = (String[]) row;
