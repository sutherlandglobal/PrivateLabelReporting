...
public final class SoldIssues extends Report
{
	...
	protected ArrayList<String[]> runReport() 
	{
		...
		for(String[] row:  dbConnection.runQuery(ordersQuery))
		{
			...
			{
				roster.getUser(userID).addAttr(SOLD_ISSUES_ATTR);
				roster.getUser(userID).addData(SOLD_ISSUES_ATTR, soldIssues);
				...
				if(isTimeReport())
				{
					...
					reportGrainData.getUser(reportGrain).addAttr(SOLD_ISSUES_ATTR);
					reportGrainData.getUser(reportGrain).addData(SOLD_ISSUES_ATTR, soldIssues);
				}
			}
		}
		...
		if(isStackReport())
		{
			...
			for(String id : roster.getUsers().keySet())
			{
				...
				if(!user.addAttr(SOLD_ISSUES_ATTR))
				{
					numIssues = stats.getTotal(user.getAttrData(SOLD_ISSUES_ATTR));
				}
		...
		else if (isTimeReport())
		{
			...
			{
				numIssues = stats.getTotal(reportGrainData.getUser(grain).getAttrData(SOLD_ISSUES_ATTR));
