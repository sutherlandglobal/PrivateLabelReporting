	...
	protected ArrayList<String[]> runReport()
	{
		...
		if(isStackReport() )
		{
			...
			for(String id : roster.getUsers().keySet())
			{
				...
				{
					caseCount = user.getAttrData(CASE_IDS_ATTR).size();
				}
				
				if(stack.containsKey(fullName))
				{
					stack.put(fullName, stack.get(fullName) + caseCount);
				}
				else
				{
					stack.put(fullName, caseCount);
				}
			}
		...
		else if(isTimeReport())
		{
			for(String user : reportGrainData.getUserList())
			{
				...
				{
					caseCount = reportGrainData.getUser(user).getAttrData(CASE_IDS_ATTR).size();
				}

				retval.add(new String[]{user, "" + caseCount }) ;
			}
		}
