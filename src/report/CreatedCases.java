...
public final class CreatedCases extends Report
{
	...
	protected ArrayList<String[]> runReport() 
	{
		...
		String userID, reportGrain, createdCaseIDs, date;
		...
		for(String[] row:  dbConnection.runQuery(query))
		{
			date = row[0];
			userID = row[1];	
			createdCaseIDs = row[2];

			if(roster.getUser(userID) != null )
			{
				roster.getUser(userID).addAttr(CREATED_CASES_ATTR);
				roster.getUser(userID).addData(CREATED_CASES_ATTR, createdCaseIDs);
				...
				if(isTimeReport())
				{
					...
					reportGrainData.getUser(reportGrain).addData(CREATED_CASES_ATTR, createdCaseIDs);
				}
			}
		}

		//format the output
		double numCreatedCases;
		if(isStackReport())
		{
			...
			for(String id : roster.getUsers().keySet())
			{
				...
				if(!user.addAttr(CREATED_CASES_ATTR))
				{
					numCreatedCases = user.getAttrData(CREATED_CASES_ATTR).size();
				}
				...
				{
					stack.put(fullName, stack.get(fullName) + numCreatedCases);
				}
				else
				{
					stack.put(fullName, numCreatedCases);
				}
			}
		...
		else if (isTimeReport())
		{
			...
			{
				numCreatedCases = reportGrainData.getUser(grain).getAttrData(CREATED_CASES_ATTR).size();
