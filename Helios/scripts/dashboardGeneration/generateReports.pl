...
while( `/bin/ls /opt/tomcat/webapps/birt-viewer/WEB-INF/lib/Helios/ | /bin/grep \.lck\$` && $attempts < $MAX_ATTEMPTS)
{
...
die "FAILED, timeout waiting for database availability\n" unless $attempts < $MAX_ATTEMPTS;

my $confDir = "/opt/tomcat/webapps/birt-viewer/WEB-INF/lib/Helios/conf/dashboard";
my $targetDir = "/opt/tomcat/webapps/birt-viewer/WEB-INF/lib/Helios/dashboards/dump/";
